# -*- coding: utf-8 -*-
"""
Created on Thu Mar 21 09:03:21 2019
...
pkl_list = ['Data/train' + str(i) + '.pkl' for i in range(1, 6)]
...
#Print message to give data about how many batches are in the data
print("Testing set: Data samples: %d, Total number of points: %d"%(validation_data_generator.total_samples, validation_data_generator.total_number_of_datapoints()))
...
with tf.variable_scope('BLSTM1') as scope: 
    lstm_fw_cell1 = tf.nn.rnn_cell.LSTMCell(n_neurons_in_h1)           
    lstm_bw_cell1 = tf.nn.rnn_cell.LSTMCell(n_neurons_in_h1) 
...
with tf.variable_scope('BLSTM2') as scope: 
    lstm_fw_cell2 = tf.nn.rnn_cell.LSTMCell(n_neurons_in_h2)
    lstm_bw_cell2 = tf.nn.rnn_cell.LSTMCell(n_neurons_in_h2)
...
# 6400x129
a = tf.nn.relu((tf.matmul(out_concate, W0)+b0), name="activationOutputLayer")
...
###################################################################  
    ...
    try:
        ...
        #Append mean of loss  over batch to accuracy_loss results
            ...
            print("loss= ",np.mean(intermediate_loss))
...
#del loss1, intermediate_accuracy, intermediate_loss, epoch#,i
...
#wavfile1 = "Audio/Train/DR1/MWDK0/SA1_.wav"
#wavfile2 = "Audio/Train/DR1/MDPK0/SI552_.wav"
...
recover_spectrogram=split_spectrogram * np.exp(1.j * testdata['phases'] )

#Apply ibm to spectrogram
split_spectrogram1 = np.multiply(recover_spectrogram, np.transpose(ibm))
split_spectrogram2 = np.multiply(recover_spectrogram, np.transpose((np.invert(ibm.astype(bool))).astype(int)))
...
plt.savefig("Signal performance.png")
